...
let handler = async (m, { conn, args, usedPrefix, command }) => {
	...
	try {
		...
		if (/webp/g.test(mime)) {
			...
			stiker = await addExif(img, global.packname || '', global.stickauth || '')
		} else if (/image/g.test(mime)) {
			let img = await q.download?.()
			stiker = await createSticker(img, false, global.packname, global.stickauth)
		} else if (/video/g.test(mime)) {
		//	if ((q.msg || q).seconds > 10) throw 'Max 10 seconds!'
			let img = await q.download?.()
			stiker = await mp4ToWebp(img, { pack: global.packname, author: global.stickauth })
