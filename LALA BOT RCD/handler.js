...
FOLLOW INSTAGRAM RECODER @azrillikethis
INGIN BERTEMAN/ MAU SC ? CHAT 6285157457662*/
...
if (!isNumber(user.exp))
	...
	if (!isOwner && !m.fromMe && opts['self']) return;
        ...
        conn.readMessages([m.key]) //AUTOREAD
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        case 'add':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/e90a021c4d8ff6b8101da.jpg'
                    try {
                        ...
                        //nini = this.sendMessage(id, { text: text }, { mentions: [user] })
                    }
                }
            }
            break
        ...
        case 'demote':
            ...
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user tidak lagi menjadi admin')
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        unreg: '*ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ʀᴇɢɪsᴛᴇʀᴇᴅ ʏᴇᴛ* • ᴋᴇᴛɪᴋ  @verify  ᴜɴᴛᴜᴋ ʙɪsᴀ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ғɪᴛᴜʀ ɪɴɪ', 
