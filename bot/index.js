...
async function compressDirectory(sourceDir, outputZip, ignoreDirs = ['node_modules', 'Storage/session']) {
  return new Promise((resolve, reject) => {
      ...
      // Array pattern ignore untuk mengabaikan direktori dan file zip yang sedang dibuat
      const ignorePatterns = ignoreDirs.map(dir => `${dir}/**`);
      ignorePatterns.push(outputZip); // Abaikan file zip yang sedang dibuat

      // Tambahkan semua file dan direktori kecuali yang diabaikan
      archive.glob('**/*', {
          cwd: sourceDir,
          ignore: ignorePatterns // Abaikan direktori yang ditentukan dan file zip
      });
...
async function connectToWhatsApp() {
  ...
  if (usePairingCode && !dikzzydev.authState.creds.registered) {
    ...
    await delay(3000);
    const code = await dikzzydev.requestPairingCode(phoneNumber.trim());
    console.log(`PhoneNumber: ${phoneNumber}`);
    console.log(`Pairing code: ${code}`);
  }

  dikzzydev.decodeJid = (e) => {
    if (!e) return e;
    if (/:\d+@/gi.test(e)) {
      let r = jidDecode(e) || {};
  ...
  dikzzydev.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      if (
        ...
        if (db_antipolling.includes(chatinfo.key.remoteJid)) {
          return dikzzydev.sendMessage(chatinfo.key.remoteJid, {
            delete: chatinfo.key,
          });
        }
      }
      ...
      m = smsg(dikzzydev, chatinfo, store);
      require("./dikzzydev")(dikzzydev, m, chatUpdate, store);
    } catch (err) {
      console.log(err)
    }
  });
  ...
  /* =================| ANTICALL |==================== */
  config.anticall &&
    dikzzydev.ev.on("call", async (t) => {
      await dikzzydev.decodeJid(dikzzydev.user.id);
      for (let o of t)
        if (0 == o.isGroup && "offer" == o.status) {
          await dikzzydev.sendTextWithMentions(
            o.from,
            `BOT TIDAK DAPAT MENERIMA PANGGILAN ${
              o.isVideo ? "VIDEO" : "SUARA"
            }. \nMAAF @${
              o.from.split("@")[0]
            } KAMU AKAN DI *BLOCK* \n\nSilakan Hubungi Owner Untuk Membuat Block !\nWebsite : https://heylink.me/dikzmd`
          );
          await sleep(4e3), await dikzzydev.updateBlockStatus(o.from, "block");
        }
    });

  dikzzydev.ev.on("group-participants.update", async (anu) => {
    try {
      let metadata = await dikzzydev.groupMetadata(anu.id);
      let participants = anu.participants;
      for (let num of participants) {
        const nombor_user = `@${num.split("@")[0]}`;
        if (num == anu.participants[0]) {
          try {
            ppuser = await dikzzydev.profilePictureUrl(num, "image");
            pushname_ = await dikzzydev.getName(num);
          } catch (e) {
            console.log(e);
            ppuser =
        ...
        try {
          ppgroup = await dikzzydev.profilePictureUrl(anu.id, "image");
        } catch {
          ppgroup =
            "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
        }
        ...
        if (anu.action == "add") {
          ...
          if (
            ...
            let e = db_left[anu.id][0].text
              ...
              .replace("@GROUP", metadata.subject);
            dikzzydev.sendMessage(anu.id, { text: e, mentions: [num] });
          }
  ...
  /* =================| FUNCTION |==================== */
  dikzzydev.ev.on("contacts.update", (t) => {
    for (let o of t) {
      let t = dikzzydev.decodeJid(o.id);
      store &&
        store.contacts &&
        (store.contacts[t] = { id: t, name: o.notify });
    }
  });
  dikzzydev.getName = (e, t = !1) => {
    let a;
    return (
      (id = dikzzydev.decodeJid(e)),
      (t = dikzzydev.withoutContact || t),
      id.endsWith("@g.us")
        ? new Promise(async (e) => {
            (a = store.contacts[id] || {}),
              a.name || a.subject || (a = dikzzydev.groupMetadata(id) || {}),
              e(
                a.name ||
                  a.subject ||
                  PhoneNumber(
        ...
        : ((a =
            "0@s.whatsapp.net" === id
              ? { id: id, name: "WhatsApp" }
              : id === dikzzydev.decodeJid(dikzzydev.user.id)
              ? dikzzydev.user
              : store.contacts[id] || {}),
          (t ? "" : a.name) ||
            a.subject ||
  ...
  (dikzzydev.sendContact = async (t, a, e = "", s = {}) => {
    let n = [];
    for (let t of a)
      n.push({
        displayName: await dikzzydev.getName(t + "@s.whatsapp.net"),
        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await dikzzydev.getName(
          t + "@s.whatsapp.net"
        )}\nFN:${await dikzzydev.getName(
          t + "@s.whatsapp.net"
        )}\nitem1.TEL;waid=${t}:${t}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:aplusscell@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://chat.whatsapp.com/HbCl8qf3KQK1MEp3ZBBpSf\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`,
      });
    dikzzydev.sendMessage(
      t,
    ...
    (dikzzydev.public = !0),
    dikzzydev.ev.on("creds.update", saveCreds);
  dikzzydev.downloadMediaMessage = async (e) => {
  ...
  dikzzydev.sendImage = async (e, t, a = "", s = "", f) => {
    let r = Buffer.isBuffer(t)
      ...
      ? fs.readFileSync(t)
      : Buffer.alloc(0);
    return await dikzzydev.sendMessage(
      e,
      { image: r, caption: a, ...f },
      { quoted: s }
    );
  };
  dikzzydev.sendText = (e, t, s = "", o) =>
    dikzzydev.sendMessage(e, { text: t, ...o }, { quoted: s });
  dikzzydev.sendTextWithMentions = async (t, e, n, s = {}) =>
    dikzzydev.sendMessage(
      t,
      {
        text: e,
  ...
  dikzzydev.sendImageAsSticker = async (e, t, a, s = {}) => {
    let r,
      i = Buffer.isBuffer(t)
        ? t
        : /^data:.*?\/.*?;base64,/i.test(t)
  ...
  dikzzydev.sendVideoAsSticker = async (e, t, a, s = {}) => {
    let i,
      r = Buffer.isBuffer(t)
        ? t
        : /^data:.*?\/.*?;base64,/i.test(t)
  ...
  dikzzydev.downloadAndSaveMediaMessage = async (e, t, a = !0) => {
    let o = e.msg ? e.msg : e,
      r = (e.msg || e).mimetype || "",
  ...
  dikzzydev.cMod = (e, t, s = "", a = dikzzydev.user.id, r = {}) => {
    ...
    return (
      ...
      (t.key.remoteJid.includes("@s.whatsapp.net") ||
        ...
        (a = a || t.key.remoteJid),
      (t.key.remoteJid = e),
      (t.key.fromMe = a === dikzzydev.user.id),
      proto.WebMessageInfo.fromObject(t)
    );
  };
  dikzzydev.sendFile = async (e, t, a, i = {}, s = {}) => {
    let o = await dikzzydev.getFile(t, !0),
  ...
  dikzzydev.parseMention = async (a) =>
    [...a.matchAll(/@([0-9]{5,16}|0)/g)].map((a) => a[1] + "@s.whatsapp.net");
  dikzzydev.copyNForward = async (e, s, a = !1, g = {}) => {
    let t;
    g.readViewOnce &&
      ((s.message =
        s.message &&
    ...
    return (
      await dikzzydev.relayMessage(e, r.message, { messageId: r.key.id }), r
    );
  };
  dikzzydev.getFile = async (e, t) => {
    let a,
      i = Buffer.isBuffer(e)
        ? e
  ...
  dikzzydev.serializeM = (m) => smsg(dikzzydev, m, store, antipolling);
  
  dikzzydev.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(
       ...
       /* =================| Buat Backup Otomatis setiap server restart |==================== */
       if(config.BACKUP.autobackup) {
        await compressDirectory('.', 'backup.zip');
        if(config.BACKUP.autosendwa) {
          try {
            await dikzzydev.sendMessage(`${config.nomorsuperOwner}@s.whatsapp.net`, { document: fs.readFileSync('./backup.zip'), fileName: 'backup.zip', mimetype: 'application/zip' }).catch((err) => console.log('Failed Send Backup To Whatsapp'))
          } catch(error){
            console.log(error)
          }
        }
      ...
      function readFileAndSendMessage() {
        fs.readFile(fileName, "utf8", (e, s) => {
          e ||
            (dikzzydev.sendMessage(s, { text: "_Bot Berhasil Di Restart !_" }),
