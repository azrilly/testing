let handler = async (m, { conn }) => {
  ...
  if (typeof werewolf.players == "undefined" || Object.keys(werewolf.players).length == 0) {
    conn.reply(m.chat, "_Tidak ada pemain dalam game ini !_", m)
    return
  }
  ...
  conn.reply(m.chat, `Ada ${players.length} pemain dalam game ini, termasuk ${villagerCount} penduduk desa dan ${wolfCount} Werewolf:\n\n${playerList}`, m) // mengubah format pesan agar lebih mudah dibaca
}
